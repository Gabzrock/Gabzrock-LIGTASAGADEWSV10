...
function renderAwsAdvisoriesTable() {
    ...
    Object.keys(groupedData).sort().forEach(category => {
        ...
        sortedGroup.forEach(station => {
            ...
            if (level === 1) { bgColor = '#f1c40f'; textColor = '#333'; levelText = 'Level 1'; } 
            else if (level === 2) { bgColor = '#e67e22'; textColor = '#fff'; levelText = 'Level 2'; } 
            else if (level === 3) { bgColor = '#e74c3c'; textColor = '#fff'; levelText = 'Level 3'; }
    ...
    downloadAwsAdvisoriesPdfBtn.onclick = function() {
        ...
        fullHtml += `
            <div style="margin-top: 30px; padding-top: 15px; border-top: 1px solid #aaa; text-align: center; font-size: 0.9rem; color: #333; page-break-inside: avoid;">
                <p style="margin: 0; font-weight: bold;">This report is generated by LIGTAS-AGAD Program RILEWS Funded by DOST Monitored by PCIEERD, implemented by UPLB-SESAM</p>
            </div>
        `;
